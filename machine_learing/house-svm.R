...
library(caret)
=======
>>>>>>> 99fdaf026353b2718b9c0d51c7ece1b928219366
...
# 학습데이터와 검증데이터 셋 분리
train_idx = createDataPartition(house$price, p=0.6, list=FALSE )
=======
# attach(house) # house 의 변수를 자유롭게 쓰기위해서 attach로 접근하는게 좋네. 불편한거 해결쓰
...
train_df = train_df[, -nearZeroVar(house)]


# 조절인자 튜닝, 10-fold-cross validation
svm_tune = tune.svm(price ~., data=train_df, gamma = 2^(-2:2), cost = 2^(-1:4), kernel='radial')
svm_tune$best.parameters
summary(svm_tune)


# 튜닝한 결과 확인 후 조절인자 조정 하고 svm모델 생성, cross-validation 설정
