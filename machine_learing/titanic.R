...
# install.packages('gridExtra') # ggplot 여러 개 합치기
...
# install.packages('ggmosaic') # ggplot 모자이크플롯
# install.packages('ROCR') # Roc커브
# install.packages('pscl') # 로지스틱 회귀분석 결정계수 확인
...
train = select(train, -PassengerId, -Ticket, -Name)
...
##### 3. Fare
filter(test, is.na(Fare)) # 확인 결과 Embarked 값이 S로 확인. 
test$Fare[is.na(test$Fare)] = mean(filter(test, Embarked=='S')$Fare, na.rm=T) # S의 평균 값으로 대체
...
# 생존과 가족
fam1 = ggplot(train, aes(FamiliySize, fill=Survived))+
  geom_bar(position='dodge')


fam2 = ggplot(train, aes(FamiliySize, fill=Survived))+
  geom_bar(position='fill')
...
##### 4. Age
# 나이의 경우 20% 정도가 데이터가 없으나 생dl존율에 영향을 미칠것으로 예상됨. 따라서 정확도를 높이기 위해 머신러닝을 통해 결측값 예측

train = filter(train, Fare<500) # 이상치 제거
train = filter(train, !Cabin=='T')

train$Cabin = factor(train$Cabin) # 값이 없는 level은 삭제
str(train)
...
custom_grid = expand.grid(mtry=2:7)
age_rf = train(Age ~., data=age_traindf_train, method='rf', trControl=fitControl, tuneGrid=custom_grid, preProc=c('scale'))
# mtry = trControl
...
glm_predict = round(predict(glm, newdata = test, type='response'))
glm_predict = as.factor(glm_predict)

confusionMatrix(glm_predict, test$Survived) # 정분류율 93%..?


# ROC AUC 확인

glm_p = predict(glm, newdata = test, type='response')
glm_pr = prediction(glm_p, test$Survived)
glm_prf = performance(glm_pr, measure = 'tpr', x.measure = 'fpr')

plot(glm_prf)


glm_auc = performance(glm_pr, measure = 'auc')
glm_auc = glm_auc@y.values[[1]]
glm_auc # AUC 판단기준 : great(0.9 ~ 1) / good(0.8 ~ 0.9) / fair(0.7~0.8) / poor(0.6 ~ 0.7) / fail(0.5 ~ 0.6)
...
svm_model = svm(Survived~., data=train, gamma=as.numeric(svm_tune$best.parameters['gamma']), cost=as.numeric(svm_tune$best.parameters['cost']), kernel='radial')
summary(svm_model)
...
confusionMatrix(svm_predict, test$Survived) # 정분류율 86%
...
rf_model = train(Survived ~., data=train, method='rf', trControl=fitControl, tuneGrid=custom_grid, preProc=c('scale'))
rf_model_p = predict(rf_model, newdata=train)

confusionMatrix(rf_model_p, train$Survived)


rf_predict = predict(rf_model, newdata=test)
confusionMatrix(rf_predict, test$Survived) # 정분류율 86%